...
单独仓库版：调用原来的 `1660667086/123` 安装脚本，你手动选择安装参数；安装完成后，再自动按服务器多 IP 生成同端口节点。
...
这个仓库不修改 `1660667086/123`。本脚本会在缺少 `ss-server` 或 `obfs-server` 时自动拉取并运行原来的安装脚本，原安装脚本里的参数由你手动选择；原脚本安装完成后，本脚本继续写入 `ss-multiip` 并自动识别服务器 IP，生成多 IP 服务。
